...
public class DatabaseHelperAccounts {
	   ...
	   public List<String> checkExistence(String username) {
		      List<String> list = new ArrayList<String>();
		      Cursor cursor = this.db.query(TABLE_ACCOUNT, new String[] { "name", "password" }, "name = '"+ username +"'", null, null, null, "name desc");
		      if (cursor.moveToFirst()) {
		        do {
		        	 list.add(cursor.getString(0));
		        	 list.add(cursor.getString(1));
